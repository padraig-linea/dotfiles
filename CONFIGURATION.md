...
| `dot_claude/settings.json` | `~/.claude/settings.json` | Claude Code global settings (model, effort level, TUI mode, theme, editor mode, statusline command reference). `~/.claude/` is Claude Code's own convention on every OS — unlike VS Code/Neovim it doesn't split by Windows-AppData vs. Linux-XDG, so no templating is needed here. |
| `dot_claude/statusline-command.sh` | `~/.claude/statusline-command.sh` | The script `settings.json`'s `statusLine.command` invokes. Plain portable bash (deliberately avoids `basename` so it also handles native Windows paths correctly) — needs `jq` on `PATH` to run. |
